class_name Joker_3_hearts extends Node2D

var joker_effect = "Sort all hearts cards in each stack."
			...
			for i in range(stack.cards_in_stack.size() - 1, -1, -1):
				for j1 in range(stack.cards_in_stack.size()): 
					for j2 in range(stack.cards_in_stack.size() - 1):
						if stack.cards_in_stack[j2].card_suit == "hearts" and stack.cards_in_stack[j2 + 1].card_suit == "hearts":
						...
						if stack.cards_in_stack.size() == 3 and stack.cards_in_stack[0].card_suit == "hearts" and stack.cards_in_stack[2].card_suit == "hearts":
