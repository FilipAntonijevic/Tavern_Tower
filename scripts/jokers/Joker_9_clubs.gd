class_name Joker_9_clubs extends Node

var joker_effect = "When you play a gem card, remove its gem and gain 2 gold."
...
func activate(_activation_window: String, deck: Deck, ui: Ui, _card: Card):
	if activation_window == _activation_window:
		if _card.sapphire or _card.topaz or _card.emerald or _card.ruby:
			...
			_card.highlight()
			highlight()
			ui.get_parent().enemy_gold += 2
			ui.get_parent().show_coins()
