class_name Joker_4_hearts extends Node2D

var joker_effect = "On cards dealt, place 1, 2, 3 of hearts."
	...
	if activation_window == _activation_window:
		...
		for stack in array_of_stacks:
					for i in range(stack.cards_in_stack.size() - 1, -1, -1): #ovako iterira niz otpozadi, sto mora, jer ako izbaci nekog keca iz steka, ostalim kartama u steku se promene indeksi
						...
						if card.card_value == 1 and card.card_suit == "hearts":
		...
		if card_found:
			...
			for stack in array_of_stacks:
						for i in range(stack.cards_in_stack.size() - 1, -1, -1): #ovako iterira niz otpozadi, sto mora, jer ako izbaci nekog keca iz steka, ostalim kartama u steku se promene indeksi
							var card = stack.cards_in_stack[i]
							if card.card_value == 2 and card.card_suit == "hearts":
								stack.move_card_from_stack_to_a_pile(deck, card)
								highlight()
								card_found = true
		...
		if card_found:
			for stack in array_of_stacks:
					for i in range(stack.cards_in_stack.size() - 1, -1, -1): #ovako iterira niz otpozadi, sto mora, jer ako izbaci nekog keca iz steka, ostalim kartama u steku se promene indeksi
						var card = stack.cards_in_stack[i]
						if card.card_value == 3 and card.card_suit == "hearts":
							stack.move_card_from_stack_to_a_pile(deck, card)
							highlight()
